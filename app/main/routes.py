...
@bp.route('/filter/', defaults={'category': None}, methods=['POST'])
@bp.route('/filter/<category>/', methods=['POST'])
def filter_posts(category):
    ...
    if category not in categories:
        ...
        if filter == "most-liked":
            query = sa.select(Post).filter_by(category = category).order_by((len(Post.likes) - len(Post.dislikes)).desc())
        if filter == "least-liked":
            query = sa.select(Post).filter_by(category = category).order_by((len(Post.likes) - len(Post.dislikes)).asc())
        if filter == "most-commented":
            query = sa.select(Post).filter_by(category = category).order_by((len(Post.comments).desc()))
    ...
    if posts.has_prev:
        ...
        prev_url = None
    return render_template('index.html', title=category, posts=posts.items, next_url=next_url, prev_url=prev_url, filter=filter)